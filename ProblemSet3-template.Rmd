...
The assumptions on logistic regression model:
1. linearity between the log odds and the predictor variables
2. independent errors
3. multicollinearity among predictors is not too high

```{r echo=FALSE, warning=FALSE, message=FALSE}
# Model diagnostics
plot(residuals.glm(model_trump, type="pearson"))
plot(residuals.glm(model_biden, type="pearson"))
...
In order to estimate the probabilities of voting for both Donald Trump and Joe Biden, we are going to perform a post-stratification analysis. In order to use this technique, we need to subdivide the population having similar characteristics into cells. Hence, we are going to create a total of 55,325 cells based off different age group, gender, race ethnicity, education attainment, household income, and state.  
...
1. Summary : Using the survey and census data obtained from Democracy Fund + UCLA Nationscape and IPUMS USA, we have predicted the popular vote outcome of the 2020 president election in USA. 
- Logistic Regression is used to predict who is more likely to be elected for the 2020 presidential election. Explanatory variables used for the logistic regression model are age_group, gender, race, education, household_income, and state. 
- (After making the models for each candidate) $y^ps$ is measured using post-stratification method to estimate the proportion ofvoters in favor of voting for each candidate.
- *discuss about the result*

2. Conclusion: To conclude, based off the estimated proportion of voters in favor of voting for Donald Trump being 0.4334 (43.34%), we predict that Trump will win the election (......)
