...
model2_t <- glm(vote_trump ~ age_group + gender + race + education + household_income, data=survey_data)

#voting for Biden
model_b <- glmer(vote_Biden ~ age_group + gender + race + education + household_income + (1|state), 
            data=survey_data, family = binomial)
model2_b <- glm(vote_Biden ~ age_group + gender + race + education + household_income + state, 
            data=survey_data)
...
ypsTrump<- 
census_data %>%
  mutate(vote_prop_t = estimate_t*n) %>%
  summarise(estimate_vote_t = sum(vote_prop_t)/sum(n))
...
census_data %>%
  mutate(alp_predict_prop2 = estimate_b*n) %>%
  group_by(state) %>%
