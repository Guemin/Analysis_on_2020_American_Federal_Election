...
2. independent errors (logistic regression requires each observation to be independtent. )  
...
ggplot(data.frame(trump_biden), aes(x=trump_biden)) +
  geom_bar()
```
Under the assumption that whoever gets a higher expected proportion for each state wins in that state, Trump is expected to win in 31 states, and Biden is expected to win in 20 states. 
...
- (After making the models for each candidate) $\hat{y}^{ps}$ is measured using post-stratification technique to estimate the proportion of voters in favor of voting for each candidate.  
