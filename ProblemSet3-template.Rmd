...
#voting for Biden
model_b <- glmer(vote_Biden ~ age_group + gender + race + education + (1|household_income), 
            data=survey_data, family = binomial)
model2_b <- glm(vote_Biden ~ age_group + gender + race + education + household_income, 
            data=survey_data)
...
census_data %>%
  ...
  gimo <- (alp_predict = sum(alp_predict_prop)/sum(n))
