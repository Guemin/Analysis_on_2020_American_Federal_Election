...
We will be using the logistic regression model and post-stratification to predict the proportion of voters who will vote for Donald Trump and Joe Biden. 

$$ log(p_{i}/1-p_{i}) = \beta_0+\beta_1  x_{age\ group} +\beta_2 x_{race}+\beta_3 x_{race}+\beta_4x_{education}+\beta_5x_{household\ income}+\beta_6x_{state}$$
```{r}
...
census_data %>%
  mutate(vote_prop = estimate*count) %>%
  group_by(age_group) %>%
  summarise(vote_props = sum(vote_prop)/sum(count))
...
#Trump
model <- lmer(vote_trump ~ gender + age_group + race + state + education + household_income + (1|state),
              data = survey_data)
summary(model)
...
head(estimate_each_state)
total_estimate
head(B_estimate_each_state)
B_total_estimate

##Total estimate of Vote for Trump is higher than Biden's estimate.
```
...
model2_t <- glm(vote_trump ~ as.factor(age_group) + as.factor(gender) + as.factor(race) + as.factor(education) + as.factor(household_income) + as.factor(state),data=survey_data, family="binomial")

#voting for Biden
#model_b <- glmer(vote_Biden ~ age_group + gender + race + education + household_income + (1|state), 
            #data=survey_data, family = binomial)
model2_b <- glm(vote_Biden ~ as.factor(age_group) + as.factor(gender) + as.factor(race) + as.factor(education) + as.factor(household_income) + as.factor(state), 
            data=survey_data, family="binomial")
